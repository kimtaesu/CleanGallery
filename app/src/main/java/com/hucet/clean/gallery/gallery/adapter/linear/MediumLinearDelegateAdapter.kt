 ...
 */
@PerActivity
class MediumLinearDelegateAdapter @Inject constructor() : AbstractDelegateAdapter {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder?, position: Int, item: Basic, glideRequests: GlideRequests?) {
        ...
        glideRequests
                .asThumbnail()
                .signature(MediaStoreSignature(item.mimeType, item.modified, item.orientation))
                .load(item.path)
